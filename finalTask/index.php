...
function insertEmployee($employeeName, $phones, $address, $email)
{
    global $dom, $xmlFilePath;
    $employee = $dom->createElement('employee');
    $employee->appendChild($dom->createElement('name', $employeeName));
...
function updateEmployee($oldEmployeeName, $newEmployeeName, $newPhones, $newAddress, $newEmail)
{
    global $dom, $xmlFilePath;
    $employees = $dom->getElementsByTagName('employee');
    foreach ($employees as $employee) {
        $nameNode = $employee->getElementsByTagName('name')->item(0);
        if ($nameNode->nodeValue == $oldEmployeeName) {
            $nameNode->nodeValue = $newEmployeeName;
            // Update phones
...
function deleteEmployee($employeeName)
{
    ...
    foreach ($employees as $employee) {
        $nameNode = $employee->getElementsByTagName('name')->item(0);
        if ($nameNode->nodeValue == $employeeName) {
            ...
            <div class="col-md-8">
                <form action="index.php" method="post" class="p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="mobilePhone" name="phones[mobile]" placeholder="Mobile">
                        <input type="text" class="form-control" id="workPhone" name="phones[work]" placeholder="Work">
                    </div>
                    <div class="mb-3">
                        <label for="address" class="form-label">Address</label>
                        <input type="text" class="form-control" id="street" name="address[street]" placeholder="Street">
                        <input type="text" class="form-control" id="buildingNumber" name="address[building_number]" placeholder="Building Number">
